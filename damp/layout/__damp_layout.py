...
class Layout:
    ...
    def step(
        ...
        *,
        ...
        if mode == "short":
            subset_limit = None
            if short_config is not None:
                subset_limit = short_config.energy_max_points
    ...
    def _collect_short_subset(
        self,
        pairs: Sequence[_ShortPair],
        *,
        max_points: int | None = None,
    ) -> list[int]:
        if not pairs:
            return []
        limit = max_points if max_points is not None and max_points > 0 else None
        subset: set[int] = set()
        truncated = False
        for pair in pairs:
        ...
        if truncated and limit is not None:
            LOGGER.event(
                "layout.energy.short.subset_cap",
    ...
    def _evaluate_pairs_short_tensor(
        ...
        *,
        ...
        if not prepared:
            return []
        subset_indices = self._collect_short_subset(
            prepared,
            max_points=subset_limit,
        )
        subset_key = tuple(subset_indices)
